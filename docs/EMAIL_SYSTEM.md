...
Set in your deployment platform runtime environment (or `.env` locally):
...
- **Object storage:** For uploaded email images, configure `STORAGE_PROVIDER` plus either `SPACES_*` or `GCS_*`; see [CI_CD_VARIABLES.md](CI_CD_VARIABLES.md).
...
For env and deploy: [CI_CD_VARIABLES.md](CI_CD_VARIABLES.md), [`apps/server/.env.example`](../apps/server/.env.example), [`apps/site/.env.example`](../apps/site/.env.example), [`apps/admin/.env.example`](../apps/admin/.env.example).
