...
- **ensureAbsoluteImage** in `apps/client/src/components/SEO.jsx`: All relative image paths are turned into absolute URLs using `siteUrl`; missing/empty image falls back to default (1200×630).
- **Static fallback** in `apps/client/index.html`: OG meta matches home defaults (description, default image, 1200×630). React Helmet overwrites these when the app loads.
...
- **About logo**: Path `/logos/file-2qgRiQ7eUZ1uhx7Xfasq3P-The HBM LOGO.png` documented; must exist in `apps/client/public/logos/` at build/deploy.
...
- **Debug logs**: Both `console.log` calls in `apps/client/src/utils/analytics.js` are wrapped in `if (import.meta.env.DEV)` so they do not run in production builds.

### Deployment readiness

- **apps/client/vite.config.js**: Build uses default `base: '/'`; manual chunks for three, react, animation, lucide; `chunkSizeWarningLimit: 1000`.
...
| Issue | Fix |
|-------|-----|
| Console logs in analytics in production | Guarded with `import.meta.env.DEV` in `apps/client/src/utils/analytics.js`. |
| No canonical/siteUrl comment in SEO | Added comment above `siteUrl` and note for OG image 1200×630 in `apps/client/src/components/SEO.jsx`. |
| Hardcoded strings (EventDetails, About eyebrow, PageErrorBoundary, GlobeDemo) | New keys in `apps/client/src/i18n/translations.js` and components switched to `t(..., lang)`. |
...
| About logo path not documented | Comment in About.jsx that logo must exist in `apps/client/public/logos/` at deploy. |
...
- **Logo file on Hostinger**: Ensure `apps/client/public/logos/file-2qgRiQ7eUZ1uhx7Xfasq3P-The HBM LOGO.png` (or the same path you deploy) exists after upload.
...
- `apps/client/src/utils/analytics.js` — DEV-guard for console.log.
- `apps/client/src/components/SEO.jsx` — siteUrl and 1200×630 comments; getBasePath for about.
- `apps/client/src/i18n/translations.js` — New keys for events, common, about, contact, form (SEO and UI).
- `apps/client/src/pages/EventDetails.jsx` — i18n for loading/not-found/back; lang-aware SEO.
- `apps/client/src/pages/About.jsx` — Eyebrow from translations; logo path comment.
- `apps/client/src/components/PageErrorBoundary.jsx` — ErrorFallback with i18n.
- `apps/client/src/components/ui/GlobeDemo.jsx` — Loading text from translations.
- `apps/client/src/pages/Contact.jsx` — SEO title/description from translations; useT vs t usage fixed.
- `apps/client/src/pages/Events.jsx` — SEO title/description from translations.
- `apps/client/src/pages/EventRegister.jsx` — SEO title/description from translations.
- `apps/client/src/pages/CookiePolicy.jsx` — SEO title/description from translations.
- `apps/client/src/pages/LegalPage.jsx` — SEO description from translations.
- `apps/client/src/pages/TeamMember.jsx` — **New**: team member profile page with per-member SEO.
- `apps/client/src/App.jsx` — Route `/about/team/:slug` and TeamMember import.
