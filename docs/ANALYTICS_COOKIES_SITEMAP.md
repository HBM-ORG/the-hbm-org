...
- **איפה מוגדר:** התג טעון ב־`index.html`, וה־config (שליחת נתונים) רץ מ־`src/utils/analytics.js` **אחרי** שהמשתמש מאשר cookies (Analytics).
...
- **איפה מוגדר:**  
  ...
  - סקריפט ליצירה/עדכון: `npm run generate-sitemap` (מריץ את `scripts/generate-sitemap.js`).

- **robots.txt:**  
  `public/robots.txt` מפנה ל-Sitemap:  
  `Sitemap: https://www.thehbm.org/sitemap.xml`

- **מפת אתר ויזואלית:**  
  קובץ `public/visual-sitemap.html` (מפת דפים ויזואלית לניווט פנימי).
...
| **Sitemap XML** | `/sitemap.xml` באתר (או `public/sitemap.xml` בקוד) |
