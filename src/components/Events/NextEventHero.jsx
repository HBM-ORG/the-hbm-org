...
import { ArrowRight, CheckCircle, MapPin, ChevronDown, Edit3, Settings, Star, Sparkles } from 'lucide-react';
...
const NextEventHero = ({ event, isEditable = false, onUpdate = () => {}, onUpload = () => {}, visuals = {} }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] font-['Sora'] pb-20">
      ...
      <div className="relative w-full flex flex-col items-center justify-center text-center overflow-hidden" style={{ minHeight: '850px' }}>
        ...
        <div className="absolute inset-0 z-0 h-full w-full bg-gray-900">
            {event.heroVideo ? (
                ...
                <img src={event.heroImage || event.image || '/assets/default-hero.jpg'} alt="Event Background" className="w-full h-full object-cover transition-all duration-300"
                    style={{ filter: `brightness(${brightness}%) blur(${blur}px)`, transform: `scale(${videoScale})` }} />
            )}
        ...
        <div className="relative z-20 max-w-4xl px-6 w-full flex flex-col items-center h-full justify-center pt-20 pb-40">
            ...
            {!isPast && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="w-full max-w-2xl mx-auto flex flex-col items-center">
                        ...
                        {!isRegisterOpen ? (
                            <div className="flex flex-col items-center">
                                ...
                                <div className="flex items-center gap-2 text-white/60 text-sm font-bold uppercase tracking-widest mb-8">
                                    <MapPin className="w-3.5 h-3.5 text-hbm-orange" /> {event.location}
                                </div>
                                <button 
                                    onClick={() => {
                                ...
                                {submitStatus === 'success' ? (
                                    <div className="text-center py-4 flex flex-col items-center">
                                        ...
                                        <p className="text-white/60 text-sm mb-6">
                                            {t({en: "Check your email for details.", he: "בדקו את המייל לפרטי הרישום."}, lang)}
                                        </p>
                                        ...
                                        <CalendarDropdown 
                                          eventData={{
                                            ...
                                            location: event.locationParams?.addressText || event.location,
                                    ...
                                    <>
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <input 
                                                type="text" 
                                                placeholder={t({en: "Full Name", he: "שם מלא"}, lang)} 
                                                required 
                                            ...
                                            <input 
                                                type="tel" 
                                                placeholder={t({en: "Phone Number", he: "מספר טלפון"}, lang)} 
                                                required 
                                                className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/50 focus:outline-none focus:border-[#F07B3C] focus:bg-black/50 transition-all text-sm backdrop-blur-sm"
                                        ...
                                        <input 
                                            type="email" 
                                            placeholder={t({en: "Email Address", he: "כתובת אימייל"}, lang)} 
                                            required 
                                            className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/50 focus:outline-none focus:border-[#F07B3C] focus:bg-black/50 transition-all text-sm backdrop-blur-sm"
                                        ...
                                        <div className="relative">
                                            <select 
                                                ...
                                                required
                                            >
                                                <option value="" disabled className="text-gray-500">{t({en: "How did you hear about us?", he: "איך שמעת עלינו?"}, lang)}</option>
                                        ...
                                        <button type="submit" disabled={submitStatus === 'submitting'}
                                            ...
                                            {submitStatus === 'submitting' ? t({en: 'Reserving...', he: 'שריינו לי...'}, lang) : t({en: 'Complete Registration', he: 'השלם הרשמה'}, lang)}
                                        </button>
                                    </>
                                )}
                            </form>
          ...
          <div className="container mx-auto px-6 -mt-16 relative z-30">
              <div className="grid md:grid-cols-3 gap-8">
                  {promoBubbles.map((bubble, idx) => (
                      <div key={idx} className="bg-white p-12 rounded-[2rem] text-center shadow-xl shadow-gray-200/50 hover:scale-105 transition-all duration-300 border border-gray-100 flex flex-col items-center justify-center min-h-[280px]">
                          ...
                          <h3 className="text-2xl font-black text-hbm-dark mb-3 font-['Sora'] tracking-tight">{t(bubble.title, lang)}</h3>
                          <p className="text-hbm-gray text-base leading-relaxed font-['Sofia_Sans'] font-medium">{t(bubble.desc, lang)}</p>
                      </div>
                  ))}
              </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto px-6 text-center">
              <div className="mb-20">
                  ...
                  <h2 className="text-5xl md:text-7xl font-black text-hbm-dark tracking-tighter mb-8 leading-[0.9]">
                      {t({ en: 'What awaits you at the event?', he: 'מה מחכה לכם באירוע?' }, lang)}
                  </h2>
            ...
            <div className="max-w-5xl mx-auto bg-white rounded-[3rem] p-16 md:p-24 text-center shadow-2xl shadow-[#6160AB]/5 border border-white relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    ...
                    <h3 className="text-4xl md:text-5xl font-black text-hbm-dark mb-8 tracking-tighter">
                        {t(partnership.title || { en: 'Our Community Partners', he: 'השותפים הקהילתיים שלנו' }, lang)}
                    </h3>
                    <p className="text-xl text-hbm-gray mb-12 max-w-2xl mx-auto font-['Sofia_Sans'] font-medium leading-relaxed">
                        {t(partnership.text || { en: 'We unite forces with the best communities to bring you the highest quality networking.', he: 'אנחנו מאחדים כוחות עם הקהילות הטובות ביותר כדי להביא לכם את הנטוורקינג האיכותי ביותר.' }, lang)}
                    </p>
                    ...
                    <div className="flex flex-wrap justify-center items-center gap-12 mb-16 px-4">
                        {(event.partners || []).length > 0 ? (
                            event.partners.map((partner, pidx) => (
                                <div key={pidx} className="h-20 grayscale hover:grayscale-0 transition-all duration-500 flex items-center justify-center group/logo">
                                    {partner.logo ? (
                                        <img src={partner.logo} alt={partner.name} className="max-h-full object-contain filter drop-shadow hover:scale-110 transition-transform" />
                                    ) : (
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">{partner.name}</span>
                                        </div>
                                    )}
                                </div>
                            ))
      ...
      {freeText && (
          <div className="max-w-5xl mx-auto px-6 py-24 text-center">
              ...
              <h3 className="text-4xl md:text-5xl font-black text-hbm-dark mb-10 tracking-tighter">
                  {t({ en: 'Good to know before you come', he: 'חשוב לדעת לפנ״י שמגיעים' }, lang)}
              </h3>
          ...
          <div className="container mx-auto px-6 py-20">
              <div className="grid md:grid-cols-3 gap-8">
                  {imageBubbles.map((bubble, idx) => (
                      <BubbleContainer key={idx} bgColor="white" fullHeight={false} className="!p-0 border-none overflow-hidden hover:scale-105 transition-transform duration-500 shadow-xl group">
                          <div className="h-48 w-full overflow-hidden">
                              <img src={bubble.image} alt={t(bubble.title, lang)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                          </div>
                          <div className="p-6 text-center bg-white border-t border-gray-50">
                              <h3 className="text-lg font-bold text-hbm-dark font-['Sora']">{t(bubble.title, lang)}</h3>
                          </div>
                      </BubbleContainer>
                  ))}
              </div>
          ...
          <div className="max-w-5xl mx-auto px-6">
                ...
                <div className="mb-24">
                    ...
                    <div className="rounded-[2.5rem] overflow-hidden shadow-2xl border border-white h-[450px] relative group">
                        ...
                        <div className="absolute top-4 left-4 bg-white/95 backdrop-blur px-4 py-2 rounded-lg text-xs font-bold shadow-sm flex items-center gap-2 pointer-events-none">
                            <MapPin className="w-3 h-3 text-hbm-orange"/> {event.location || 'Raanana'}
                        </div>
                    </div>
                </div>
                ...
                {event.hostNote?.message && (
                    <div className="max-w-3xl mx-auto mb-24 relative">
                        ...
                        <BubbleContainer bgColor="white" fullHeight={false} className="p-12 !rounded-[3rem] border border-white shadow-xl relative overflow-hidden">
                            ...
                            <div className="flex flex-col items-center gap-4">
                                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#6160AB] to-[#F07B3C] p-0.5 shadow-lg">
                                    <div className="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                                        {event.hostNote.avatar ? <img src={event.hostNote.avatar} className="w-full h-full object-cover" /> : <Sparkles className="w-8 h-8 text-[#6160AB]/30" />}
                                    </div>
                                </div>
                                <div className="text-center">
                                    <div className="font-black text-hbm-dark text-sm uppercase tracking-[0.2em]">{event.hostNote.author || 'The HBM Team'}</div>
                                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Event Host</div>
                                </div>
                            </div>
                        </BubbleContainer>
                    </div>
                ...
                {event.faqs && event.faqs.length > 0 && (
                    <div className="max-w-3xl mx-auto">
                         <div className="text-center mb-12">
                             <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-4">Support</h4>
                        ...
                        <div className="space-y-4" dir={lang === 'he' ? 'rtl' : 'ltr'}>
                            {event.faqs.map((faq, idx) => (
                                <div key={idx} className="bg-white/60 backdrop-blur-sm rounded-[2rem] overflow-hidden border border-white hover:border-[#6160AB]/20 transition-all hover:shadow-lg">
                                    <button 
                                        onClick={() => toggleFaq(idx)}
                                        className="w-full p-8 flex justify-between items-center text-left hover:bg-white transition-colors"
                                    >
                                        <span className={`font-extrabold tracking-tight transition-colors ${activeFaq === idx ? 'text-[#6160AB]' : 'text-hbm-dark'}`}>{faq.question}</span>
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${activeFaq === idx ? 'bg-[#6160AB] text-white rotate-180' : 'bg-gray-100 text-gray-400'}`}>
