 ...
 *
 * Usage:
 *   npx tsx scripts/one-off/migrate-json-to-db.ts [--dry-run]
 *
 * Options:
...
const SUPPRESSION_PATH = path.join(
  PROJECT_ROOT,
  'apps',
  'client',
  'src',
...
const VIDEO_EVENT_PATH = path.join(
  PROJECT_ROOT,
  'apps',
  'client',
  'src',
...
const HOW_IT_WORKS_PATH = path.join(
  PROJECT_ROOT,
  'apps',
  'client',
  'src',
...
async function migrateCampaigns(): Promise<{ count: number; errors: string[] }> {
  const campaigns = loadJsonFile(CAMPAIGNS_PATH);
  if (!Array.isArray(campaigns)) {
    return { count: 0, errors: ['Invalid or missing campaigns.json'] };
  }
  ...
  for (const campaign of campaigns) {
    try {
      ...
      count++;
      console.log(`  Migrated campaign: ${legacyId}`);
    } catch (error) {
      const errorMsg = `Failed to migrate campaign ${campaign?.id || count}: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      console.error(`  ERROR: ${errorMsg}`);
    }
  }
...
async function migrateSuppressionList(): Promise<{ count: number; errors: string[] }> {
  const entries = loadJsonFile(SUPPRESSION_PATH);
  if (!Array.isArray(entries)) {
    return { count: 0, errors: ['Invalid or missing suppression.json'] };
  }

  const errors: string[] = [];
  let count = 0;

  for (const rawEmail of entries) {
    try {
      ...
      count++;
      console.log(`  Migrated suppression: ${email}`);
    } catch (error) {
      const errorMsg = `Failed to migrate suppression entry: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      console.error(`  ERROR: ${errorMsg}`);
    }
  }
...
async function migrateContentEntries(): Promise<{ count: number; errors: string[] }> {
  const errors: string[] = [];
  let count = 0;
  ...
  const targets: Array<{
    ...
    { key: 'videoEvent', publishedPath: VIDEO_EVENT_PATH },
    {
      key: 'howItWorks',
      publishedPath: HOW_IT_WORKS_PATH,
      draftPath: HOW_IT_WORKS_STAGING_PATH,
    },
    { key: 'knowledgeBase', publishedPath: KNOWLEDGE_BASE_PATH },
  ];

  for (const target of targets) {
    try {
      const published = loadJsonFile(target.publishedPath) || {};
      const draft = target.draftPath ? loadJsonFile(target.draftPath) : undefined;
      const isLocked =
        typeof published?.isLocked === 'boolean'
          ? published.isLocked
          : typeof draft?.isLocked === 'boolean'
            ? draft.isLocked
            : false;
      ...
      count++;
      console.log(`  Migrated content entry: ${target.key}`);
    } catch (error) {
      const errorMsg = `Failed to migrate content entry ${target.key}: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      console.error(`  ERROR: ${errorMsg}`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    const campaignResult = await migrateCampaigns();
    console.log(`Campaigns: ${campaignResult.count} migrated`);
    console.log(`Errors: ${campaignResult.errors.length}`);
    console.log('');

    // Migrate suppression list
    console.log('--- Migrating Suppression List ---');
    const suppressionResult = await migrateSuppressionList();
    console.log(`Suppression entries: ${suppressionResult.count} migrated`);
    console.log(`Errors: ${suppressionResult.errors.length}`);
    console.log('');

    // Migrate remaining content JSON
    console.log('--- Migrating Content Entries ---');
    const contentResult = await migrateContentEntries();
    console.log(`Content entries: ${contentResult.count} migrated`);
    ...
    console.log(`Campaigns: ${campaignResult.count}`);
    console.log(`Suppression Entries: ${suppressionResult.count}`);
    console.log(`Content Entries: ${contentResult.count}`);
    console.log('');

    const totalErrors =
      eventResult.errors.length +
      siteResult.errors.length +
      automationResult.errors.length +
      campaignResult.errors.length +
      suppressionResult.errors.length +
