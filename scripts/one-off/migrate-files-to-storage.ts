 ...
 *
 * Usage:
 *   npx tsx scripts/one-off/migrate-files-to-storage.ts [--dry-run] [--skip-db-update] [--update-db-only]
 *
 * Options:
 *   --dry-run         Preview uploads without actually uploading
 *   --skip-db-update  Only upload files, don't update database records
 *   --update-db-only  Reuse scripts/one-off/url-mapping.json to update DB without uploading
 */
...
import { getStorageAdapter } from '../../apps/server/src/storage/adapter.js';
...
// Paths
const ASSETS_DIR = path.join(PROJECT_ROOT, 'apps', 'client', 'public', 'assets');
