 ...
 *
 * Usage:
 *   npx tsx scripts/migrate-storage-provider.ts --from spaces --to gcs
 *   npx tsx scripts/migrate-storage-provider.ts --from gcs --to spaces --dry-run
 *   npx tsx scripts/migrate-storage-provider.ts --from spaces --to gcs --update-db-only
 *
 * Notes:
 * - Uses existing SPACES_* and GCS_* env vars for both providers.
 * - Saves URL mapping to scripts/provider-url-mapping-<from>-to-<to>.json
