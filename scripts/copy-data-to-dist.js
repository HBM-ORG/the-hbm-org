/**
 * After vite build: copy all JSON data into dist/data/ for static hosting.
 * - data/site-configs.json (team, partners, testimonials)
 * - public/data/*.json (e.g. events.json if present)
...
const toCopy = [
  ...
  { src: path.join(projectRoot, "public", "data", "events.json"), name: "events.json" },
];
...
try {
  ...
  for (const { src, name } of toCopy) {
    if (fs.existsSync(src)) {
      ...
      console.log("copy-data-to-dist: copied", name, "→ dist/data/");
    } else {
