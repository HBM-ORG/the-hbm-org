...
const ASSETS_BASE_URL = (
  process.env.ASSETS_BASE_URL || "https://thehbm.org"
).replace(/\/$/, "");
...
async function uploadBufferViaFtp(buffer, remotePath) {
  ...
  try {
    const secure =
      process.env.FTP_SECURE === "1" || process.env.FTP_SECURE === "true";
    await client.access({
      host,
      user,
      password: pass,
...
const EMAIL_ASSETS_DIR = path.join(PROJECT_ROOT, "public", "assets", "emails");
...
app.post(
  ...
  async (req, res) => {
    ...
    const hasFtp =
      ...
      process.env.FTP_PASS !== undefined;
    if (hasFtp) {
      ...
      if (!ok) {
        return res.status(500).json({
          error:
            "Upload to Hostinger failed. Set FTP_HOST, FTP_USER, FTP_PASS on Render.",
        });
      }
...
// 2b. Upload General Asset (Video, Partners, etc.) — FTP when configured, else local (so Hero Cinema works in dev)
...
app.post("/api/upload-asset", uploadMemoryAsset, async (req, res) => {
  ...
  if (!usedFtp) {
    try {
      const localDir = subfolder
        ? path.join(ASSETS_DIR, folderName, subfolder)
      ...
      return res.status(500).json({
        error: "Upload failed (FTP and local fallback). See server logs.",
      });
    }
  }
  ...
  const relativePath = `/assets/${urlPath}`;
  const fullUrl = usedFtp ? `${ASSETS_BASE_URL}/assets/${urlPath}` : null;
...
const processQueue = async (specificItemId = null) => {
  ...
  try {
    ...
    // Prefer SMTP from config file (Admin → Email Architect); fallback to env (production-safe)
    ...
    for (const item of items) {
      ...
      try {
        ...
        const renderData = {
          ...data,
          eventDate: data.date ? new Date(data.date).toLocaleDateString() : "",
          year: 2026,
        };
...
setInterval(() => {
  if (!processQueueRunning)
    processQueue().catch((err) =>
      console.error("[Email] Interval run failed:", err),
    );
}, 60000);
...
app.post("/api/smtp-check", async (req, res) => {
  ...
  try {
    ...
    if (
      /535|auth|login|invalid credentials/i.test(String(msg)) &&
      isOffice365
    ) {
      msg =
...
app.get("/api/cms/how-it-works", (req, res) => {
  try {
    if (!fs.existsSync(HOW_IT_WORKS_CONFIG_PATH))
      return res
        .status(200)
        .json({ videoSteps: [], physicalSteps: [], isLocked: false });
...
app.get("/api/cms/knowledge-base", (req, res) => {
  try {
    if (!fs.existsSync(KNOWLEDGE_BASE_CONFIG_PATH))
      return res.status(200).json({ books: [], videos: [], isLocked: false });
    const data = JSON.parse(
      fs.readFileSync(KNOWLEDGE_BASE_CONFIG_PATH, "utf8"),
    );
    res.status(200).json(data);
  } catch (err) {
...
app.get("/api/ai/ping", (req, res) =>
  res.json({ ok: true, service: "fetch-book" }),
);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[fetch-book] Google Books response not JSON:",
          parseErr.message,
        );
        googleData = {};
      }
    ...
    if (!coverUrl)
      coverUrl = await fetchOpenLibraryCover(
        title,
...
// Fallback: any other static from public
