...
export function createEmailQueueEngine({
  ...
  async function processQueue(specificItemId: string | null = null): Promise<boolean> {
    ...
    try {
      ...
      try {
        config = readJsonFile<AutomationConfig>(automationConfigPath, {});
      } catch (error) {
        console.error("[Email] processQueue: failed to load config", error);
        return false;
      }
      ...
      for (const item of items) {
        ...
        try {
          ...
          const campaign = !flow
            ? fs.existsSync(campaignsFilePath)
              ? readJsonFile<CampaignDefinition[]>(campaignsFilePath, []).find(
                  (entry) => entry.id === item.flowId,
                )
              : null
            : null;
  ...
  async function triggerAutomationByEvent(
    ...
    try {
      if (!fs.existsSync(automationConfigPath)) return;
      const config = readJsonFile<AutomationConfig>(automationConfigPath, {});
