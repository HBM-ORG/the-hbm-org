...
export const loadClarity = (consent = null) => {
  ...
  try {
    ...
    const analyticsGranted =
      ...
      (() => {
        try {
          const saved = localStorage.getItem("hbm_cookie_consent");
          return saved ? JSON.parse(saved).analytics : false;
        } catch {
          return false;
        }
      })();
    const marketingGranted =
      ...
      (() => {
        try {
          const saved = localStorage.getItem("hbm_cookie_consent");
          return saved ? JSON.parse(saved).marketing : false;
        } catch {
          return false;
        }
      })();
    ...
    if (typeof Clarity.consentV2 === "function") {
      ...
      Clarity.consent(Boolean(analyticsGranted));
    }
  } catch (error) {
    console.warn("Clarity init/consent:", error);
  }
