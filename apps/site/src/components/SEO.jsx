...
import { getAbsoluteSiteUrl, getSiteUrl } from '../utils/api';
...
function ensureAbsoluteImage(url) {
  const defaultImage = getAbsoluteSiteUrl(DEFAULT_OG_IMAGE_PATH);
  if (!url || typeof url !== 'string') return defaultImage;
  const trimmed = url.trim();
  if (trimmed.startsWith('http://') || trimmed.startsWith('https://')) return trimmed;
  return getAbsoluteSiteUrl(trimmed);
}
...
export default function SEO({
  ...
  const resolvedImage = ensureAbsoluteImage(imageProp || seo.image);
  ...
  const organizationSchema = {
    ...
    "@type": "Organization",
    "name": PUBLIC_BRAND.organizationName,
    "url": siteUrl,
    "logo": getAbsoluteSiteUrl("/assets/logo.png"),
    "sameAs": PUBLIC_BRAND.socialProfiles,
  };
  ...
  return (
    <Helmet>
      ...
      <meta property="og:site_name" content={PUBLIC_BRAND.organizationName} />
      ...
      <meta name="apple-mobile-web-app-title" content={PUBLIC_BRAND.organizationName} />
      <meta name="application-name" content={PUBLIC_BRAND.organizationName} />
